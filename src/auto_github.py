...
def save_config(repo_path, blacklist):
    config_path = get_config_path()
    validated = validate_repo_path(repo_path)
    if not validated:
        print("[!] Repo path rejected.")
        return
    repo_path = validated

    # ⬇️ Gabungkan isi lama dengan yang baru
    existing = {}
    if os.path.exists(config_path):
        try:
            with open(config_path, 'r', encoding='utf-8') as f:
...
def validate_repo_path(repo_path):
    ...
    # Tolak path root atau sistem
    if repo_path in ["C:\\", "/", os.environ.get("WINDIR", "")]:
        print("[!] Refusing root/system path.")
        return None
    # Pastikan ada folder .git
    if not os.path.isdir(os.path.join(repo_path, ".git")):
        print("[!] Invalid repo path — must contain .git/")
        return None
    return repo_path
...
def auto_github_menu(loc_base, cfg):
    ...
    if config:
        ...
        print(f"{cfg.CYAN}2.{cfg.END} Add/Change Directory / Manage Blacklist")
    ...
    blacklist = manage_blacklist(all_files, blacklist, cfg)
    save_config(repo_path, blacklist)
