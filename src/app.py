...
def check_latest_release():
    """Checks GitHub for the latest .pyz release with caching and error handling."""

    print(f"\n   {cfg.CYAN}{cfg.BOLD}--- CHECKING LATEST RELEASE ---{cfg.END}")
    print(f"   Current Version: {cfg.YELLOW}{cfg.version}{cfg.END}")

    cache_ttl = 3600  # 1 jam
    now = time.time()
    last_check = cfg.config.get("last_release_check", 0)
    cached_release = cfg.config.get("cached_release", None)

    # --- CACHE VALIDATION ---
    cached_release = cfg.config.get("cached_release", None)
    if cached_release:
        cached_release = Version(cached_release)

    try:
        req = requests.get(cfg.raw_repo_api, headers={'User-Agent': 'Mozilla/5.0'})
        ...
        changelog = cl_resp.text.splitlines()[0] if cl_resp.ok else "No changelog available."

        # --- CACHE SAVE ---
        cfg.config["cached_release"] = str(latest_version)
        cfg.config["last_release_check"] = now
        cfg.save_config()

        print(f"   {cfg.GREEN}[✓] Latest version: {latest_version}{cfg.END}")
        return latest_version, changelog

    except requests.exceptions.RequestException as e:
        print(f"   {cfg.RED}[!] Network error: {e}{cfg.END}")
        return None, None
    except Exception as e:
        print(f"   {cfg.RED}[!] Unexpected error: {e}{cfg.END}")
        return None, None
...
def main():
    ...
    while True:
        ...
        print(f"   {cfg.CYAN}[3]{cfg.END} Sync to GitHub Desktop")
        print(f"   {cfg.CYAN}[4]{cfg.END} Update {__version__} Changelog")
        print(f"   {cfg.CYAN}[5]{cfg.END} Settings")
