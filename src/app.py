...
def run_build():
    ...
    if os.path.exists("build.bat"):
        ...
        subprocess.Popen("start /min build.bat", shell=True)
        sys.exit()
    else:
...
def translator_tool_menu():
    ...
    while True:
        ...
        print(f"   [1] Almanac Comparison (Auto Add Missing)")
        print(f"   [2] Almanac Validator (Symbol Check)")
        print(f"   [3] Travel Buffs Comparison")
        print(f"   [4] String Checker (Scan All)")
...
def main():
    ...
    while True:
        clear_screen()
        print(f"\n   {cfg.GREEN}{cfg.BOLD}PVZ FUSION GLOBAL TOOL v{__version__}{cfg.END}")
        ...
        print(f"   {cfg.RED}[5]{cfg.END} Exit")
        ...
        if choice == '1':
            if is_path_valid():
                while True:
                    # Bersihkan layar sebelum menampilkan sub-menu
                    clear_screen()
                    ...
                    if sub == '1':
                        clear_screen() # Bersihkan layar sebelum masuk ke String Menu
                        string_comp.string_menu(l_p, d_p)
                        break 
                    elif sub == '2':
                        clear_screen() # Bersihkan layar sebelum masuk ke Regex Menu
            ...
            clear_screen()
            cfg.settings_menu_ui() # Full handled by settings.py
            
        elif choice == '4':
            if is_path_valid():
                clear_screen()
                auto_github.auto_github_menu(os.path.join(cfg.config["mod_path"], 'Localization'))
            else:
